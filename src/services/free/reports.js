...
import { fetchJson } from "@/services/free/http";
...
export function normalizeReport(report) {
  ...
  return {
    ...
    createdAt:
      report.createdAt || report.created_at || report.timestamp || new Date().toISOString(),
    photoUrl,
...
export async function submitPublicPetReport({
  ...
  try {
    const { res, data } = await fetchJson(PUBLIC_PET_REPORT_URL, {
      method: "POST",
      body: formData,
    });
...
export async function fetchPublicReports({ page = 1, limit = 10 } = {}) {
  ...
  try {
    ...
    const nextPage = data?.nextPage ?? (items.length === limit ? page + 1 : null);
...
export async function toggleReaction({ reportId, reactionType }) {
  try {
    const { res, data } = await fetchJson(reportReactionsUrl(reportId), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function flagReport({ reportId, reason, details }) {
  try {
    const { res, data } = await fetchJson(reportFlagUrl(reportId), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
