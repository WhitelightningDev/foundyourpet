...
This frontend is wired for Firebase Cloud Messaging (FCM) **web push**. To actually notify users when a new report is created, you also need a backend job/endpoint that sends an FCM message to your registered device tokens (web + mobile).
...
- Service worker generation for background messages: `public/firebase-messaging-sw.js` (generated by `scripts/generate-firebase-messaging-sw.cjs` on `npm start` / `npm run build`).
...
- Public report submission posts to the backend (with fallback to email draft): `src/pages/Home.jsx`.
